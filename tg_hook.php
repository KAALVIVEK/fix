...
if ((string)$chatId !== (string)TG_CHAT_ID) {
  if (preg_match('/^\/(start|whoami)/i', $text)) { tg_send($chatId, "🔒 This bot is private.\nYour chat id: <code>" . htmlspecialchars((string)$chatId, ENT_QUOTES) . "</code>"); }
  echo json_encode(['ok'=>true]);
  exit;
}

if (preg_match('/^\/(start|help)/i', $text)) { tg_send($chatId, "👋 <b>Welcome, admin</b>\nUse the keyboard or commands:\n/status, /stock, /alertson, /alertsoff", $kb); echo json_encode(['ok'=>true]); exit; }
if (preg_match('/^\/(status|Status)|^📊/u', $text)) {
  try { $conn = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME); $ok = $conn->connect_error ? '❌ DB: DOWN' : '✅ DB: OK'; $res = $conn->query("SELECT COUNT(*) AS users FROM users"); $users = ($res && ($row = $res->fetch_assoc())) ? (int)$row['users'] : 0; tg_send($chatId, "📊 <b>Status</b>\n$ok\n👤 Users: $users\n🕒 " . date('H:i:s T'), $kb); if ($conn) { $conn->close(); } } catch (Throwable $e) { tg_send($chatId, 'Error: ' . $e->getMessage(), $kb); }
  echo json_encode(['ok'=>true]); exit;
}
if (preg_match('/^\/(stock|Stock)|^📦/u', $text)) {
  try { $conn = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME); $sys = 0; $res = $conn->query("SELECT COUNT(*) AS c FROM system_keys WHERE status='Generated'"); if ($res && ($row = $res->fetch_assoc())) { $sys = (int)$row['c']; } $lines = [ '🧱 System pool: ' . $sys ]; $q = $conn->query("SELECT p.name, COUNT(k.id) AS c FROM keys_pool k JOIN products p ON p.id = k.product_id WHERE k.is_used = 0 GROUP BY k.product_id ORDER BY p.name"); if ($q) { while ($r = $q->fetch_assoc()) { $lines[] = '📦 ' . $r['name'] . ': ' . (int)$r['c']; } } tg_send($chatId, "📦 <b>Unused stock</b>\n" . implode("\n", $lines), $kb); if ($conn) { $conn->close(); } } catch (Throwable $e) { tg_send($chatId, 'Error: ' . $e->getMessage(), $kb); }
  echo json_encode(['ok'=>true]); exit;
}
if (preg_match('/^\/(alertson|Alerts ON)|^🔔/u', $text)) { $prefs = tgGetPrefs(); $prefs['alerts_enabled'] = true; tgSetPrefs($prefs); tg_send($chatId, '🔔 Alerts <b>enabled</b>.', $kb); echo json_encode(['ok'=>true]); exit; }
if (preg_match('/^\/(alertsoff|Alerts OFF)|^🔕/u', $text)) { $prefs = tgGetPrefs(); $prefs['alerts_enabled'] = false; tgSetPrefs($prefs); tg_send($chatId, '🔕 Alerts <b>disabled</b>.', $kb); echo json_encode(['ok'=>true]); exit; }
